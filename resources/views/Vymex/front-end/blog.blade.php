...
@section("meta-tags")
	...
	@section("base-styles")
		...
		loadCSS( "{{asset(Config('page-settings.blogCSS'))}}", false, "all")
	@endsection
		...
		@section('main')
			<main class="blog">
				<div class="page-wrap-p">
					...
					<div class="blog-main">
						<h1 class="blog-title">Vymex Блог</h1>
						<div class="article">
							<?php $count = 1 ?>
							@foreach($blogs as $blog)
								@if($count == 1 || $count%6 == 0)
									<div class="article-itm">
										<a href="{{route('blog-article', ['slug'=>$blog->blog_slug])}}">
											<img src="{{asset('storage/blogs/blog-' . $blog->id . '/' . $blog->blog_img)}}">
										</a>
										<div>
											<h4><a href="{{route('blog-article', ['slug'=>$blog->blog_slug])}}">{{$blog->blog_name}}</a></h4>
											<p><a href="">{{$blog_categories->where('id', $blog->blog_category)->first()->category_name}}</a></p>
										</div>
									</div>
								@else
									<div class="article-itm">
										<a href="{{route('blog-article', ['slug'=>$blog->blog_slug])}}" class="img-box">
											<img src="{{asset('storage/blogs/blog-' . $blog->id . '/small-' . $blog->blog_img)}}">
										</a>
										<div>
											<h4><a href="{{route('blog-article', ['slug'=>$blog->blog_slug])}}">{{$blog->blog_name}}</a></h4>
											<p><a href="">{{$blog_categories->where('id', $blog->blog_category)->first()->category_name}}</a></p>
	...
	<script>var scr = {"scripts":[
		...
		{"src" : "{{Config('page-settings.BlogJS')}}", "async" : false},
